...
public class SampleData {
		...
		if (freqObjects instanceof List) {
			...
			for (Object map : maps) {
				if (map instanceof Map) {
					...
					if (m.containsKey(FREQ_YEAR_KEY) && Integer.parseInt(m.get(FREQ_YEAR_KEY).toString()) == year
	...
	protected static WordRecord buildWordRecord(String word, Map<Object, Object> definition) {
		...
		return new WordRecord(word, getFrequencyByYear(wordApi, word, FREQ_YEAR), definition.get("text").toString());
	}
	...
	public static void fillSampleData(ObservableList<WordRecord> backingList) {
		...
		backingList.add(new WordRecord("Java", 179, "An island of Indonesia in the Malay Archipelago"));
		backingList.add(new WordRecord("random", 794, "Having no specific pattern, purpose, or objective"));
	}
